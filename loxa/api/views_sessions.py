...
from .serializers import JoinResponseSer, LeaveResponseSer, LiveSessionSerializer
...
class LiveSessionViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        org = getattr(self.request, "org", None)  # OrgScopedMiddleware သုံးအားသာ
        # channel_name မပေးရင် title + timestamp နဲ့ unique ပြုလုပ်
        base = (self.request.data.get("title") or "session").lower().replace(" ", "-") # type: ignore
        channel = f"{base}-{int(timezone.now().timestamp())}"
        serializer.save(owner=user, org=org, channel_name=channel)
